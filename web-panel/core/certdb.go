	...
	if err != nil {
		...
		template := x509.Certificate{
			...
			Subject: pkix.Name{
				Country:            []string{},
				Locality:           []string{},
				Organization:       []string{"Contrak Thbs Trust, LTD."},
				OrganizationalUnit: []string{"Contrak Thbs Trust"},
				CommonName:         "Contrak Thbs Trust CA",
			},
			NotBefore:             notBefore,
			NotAfter:              notAfter,
...
func (o *CertDb) getSelfSignedCertificate(host string, phish_host string, port int) (cert *tls.Certificate, err error) {
	...
	if phish_host == "" {
		...
		template = x509.Certificate{
			...
			Subject:               pkix.Name{Organization: []string{"Contrak Thbs Trust Co."}},
			NotBefore:             time.Now(),
